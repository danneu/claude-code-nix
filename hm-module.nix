# Home Manager module for Claude Code (native binary)
#
# Provides:
...
let
  ...
  package = if cfg.channel == "stable" then pkgs.claude-code-stable else pkgs.claude-code;
  jqMergeExpr =
...
in
{
  ...
  options.programs.claude-code = {
    enable = mkEnableOption "Claude Code (native binary)";
      ...
      type = types.attrsOf (types.submodule {
        options = {
          ...
          args = mkOption {
            ...
            example = [ "dlx" "@playwright/mcp@latest" "--headless" ];
          };
          url = mkOption {
            type = types.nullOr types.str;
            default = null;
    ...
    mergeStrategy = mkOption {
      type = types.enum [ "file-wins" "nix-wins" ];
      default = "nix-wins";
      description = ''
